...
control "SV-204399" do
  ...
  desc "default", "A session time-out lock is a temporary action taken when a 
    ...
    able to identify when a user's session has idled and take action to initiate the
    session lock. The session lock is implemented at the point where session activity 
    can be determined and/or controlled."
  desc "check", "Verify the operating system prevents a user from overriding a 
    ...
    so the path is \"/etc/dconf/db/local.d\". This path must be modified if a 
