control 'SV-204576' do
  title "The Red Hat Enterprise Linux operating system must limit the number of concurrent sessions to #{input('max_maxlogins_limit')} for all
  ...
  desc 'check', "Verify the operating system limits the number of concurrent sessions to '#{input('max_maxlogins_limit')}' for all accounts and/or
    account types by issuing the following command:
    # grep \"maxlogins\" /etc/security/limits.conf /etc/security/limits.d/*.conf
    * hard maxlogins #{input('max_maxlogins_limit')}
    This can be set as a global domain (with the * wildcard) but may be set differently for multiple domains.
    If the \"maxlogins\" item is missing, commented out, or the value is not set to '#{input('max_maxlogins_limit')}' or less for all domains that have
    the \"maxlogins\" item assigned, this is a finding."
  desc 'fix', "Configure the operating system to limit the number of concurrent sessions to '#{input('max_maxlogins_limit')}' for all accounts
