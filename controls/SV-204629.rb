...
control "SV-204629" do
  title "The Red Hat Enterprise Linux operating system must not have unauthorized 
    IP tunnels configured."
  desc "IP tunneling mechanisms can be used to bypass network filtering. If tunneling 
    is required, it must be documented with the Information System Security 
    Officer (ISSO)."
  desc "default", "IP tunneling mechanisms can be used to bypass network filtering. 
  ...
  desc "check", "Verify the system does not have unauthorized IP tunnels 
    ...
    ask the System Administrator if the tunnel is documented with the ISSO. 

    If \"libreswan\" is installed, \"IPsec\" is active, and an undocumented tunnel 
    is active, this is a finding."
  desc "fix", "Remove all unapproved tunnels from the system, or document them with 
    the ISSO."
  impact 0.5
