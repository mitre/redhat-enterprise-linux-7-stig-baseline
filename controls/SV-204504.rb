...
control "SV-204504" do
  title "The Red Hat Enterprise Linux operating system must shut down upon audit 
    processing failure, unless availability is an overriding concern. If 
    availability is a concern, the system must alert the designated staff 
    (System Administrator [SA] and Information System Security Officer [ISSO] 
    at a minimum) in the event of an audit processing failure."
  desc "It is critical for the appropriate personnel to be aware if a system is 
  ...
  desc "default", "It is critical for the appropriate personnel to be aware if 
    ...
    audit storage capacity of organizations (i.e., all audit data storage
    repositories combined), or both."
  desc "check", "Confirm the audit configuration regarding how auditing 
    ...
    \"failure\" is set to \"1\", the system is configured to only send 
    ...
    documented or there is no monitoring of the kernel log, this should be downgraded 
    to a CAT III finding."
  desc "fix", "Configure the operating system to shut down in the event of an 
    ...
    administration staff and ISSO staff in the event of an audit processing 
