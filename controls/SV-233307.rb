...
control "SV-233307" do
  title "The Red Hat Enterprise Linux operating system SSH daemon must prevent 
    remote hosts from connecting to the proxy display."
  desc "When X11 forwarding is enabled, there may be additional exposure to the 
    ...
    loopback address and sets the hostname part of the DIPSLAY environment variable 
    to localhost. This prevents remote hosts from connecting to the proxy display."
  desc "default", "When X11 forwarding is enabled, there may be additional exposure 
    to the server and client displays if the sshd proxy display is configured to 
    ...
    display."
  desc "check", "Verify the SSH daemon prevents remote hosts from connecting to 
    the proxy display.
  ...
  desc "fix", "Configure the SSH daemon to prevent remote hosts from connecting 
    to the proxy display.

    Edit the \"/etc/ssh/sshd_config\" file to uncomment or add the line for the 
    \"X11UseLocalhost\" keyword and set its value to \"yes\" (this file may be named 
