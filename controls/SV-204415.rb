...
control "SV-204415" do
  ...
  desc "Passwords need to be protected at all times, and encryption is the 
    ...
    they can be plainly read (i.e., clear text) and easily compromised. Passwords 
    encrypted with a weak algorithm are no more protected than if they are kept 
    in plain text."
  desc "default", "Passwords need to be protected at all times, and encryption 
    is the standard method for protecting passwords. If passwords are not 
    encrypted, they can be plainly read (i.e., clear text) and easily 
    compromised. Passwords encrypted with a weak algorithm are no more protected 
    than if they are kept in plain text."
  desc "check", "Verify the PAM system service is configured to store only 
    encrypted representations of passwords. The strength of encryption that 
    must be used to hash passwords for all accounts is SHA512.
    ...
    If the \"/etc/pam.d/system-auth\" and \"/etc/pam.d/password-auth\" configuration 
    files allow for password hashes other than SHA512 to be used, this is a finding."
  desc "fix", "Configure the operating system to store only SHA512 encrypted 
    ...
    \"authconfig\" program. The \"authconfig\" program should not be used to 
    update the configurations listed in this requirement."
  impact 0.5
 ...
 should match_pam_rule('password sufficient pam_unix.so sha512') }
      its('lines') {
 should_not match_pam_rule('password .* pam_unix.so (md5|bigcrypt|sha256|blowfish)') }
    end
