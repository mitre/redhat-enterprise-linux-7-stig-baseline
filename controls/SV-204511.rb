...
control "SV-204511" do
  title "The Red Hat Enterprise Linux operating system must be configured so that 
    the audit system takes appropriate action when the audit storage volume is full."
  desc "Taking appropriate action in case of a filled audit storage volume will 
    ...
    One method of off-loading 
    audit logs in Red Hat Enterprise Linux is with the use of the audisp-remote 
    dameon."
  desc "default", "Taking appropriate action in case of a filled audit storage 
    volume will minimize the possibility of losing audit records.
    One method of off-loading audit logs in Red Hat Enterprise Linux is with the 
    use of the audisp-remote dameon."
  desc "check", "Verify the action the operating system takes if the disk the 
    ...
    or \"halt\", or the line is commented out, ask the System Administrator to 
    indicate how the audit logs are off-loaded to a different system or storage 
    media, and to indicate the action taken when the disk is full on the remote server.

    If there is no evidence that the system is configured to off-load audit logs to 
    a different system or storage media, or if the configuration does not take 
    appropriate action when the disk is full on the remote server, this is a finding."
  desc "fix", "Configure the action the operating system takes if the disk the 
    ...
    and set it to \"syslog\", \"single\", or \"halt\", such as the following line:
  ...
  if virtualization.system.eql?('docker')
    ...
    describe parse_config_file('/etc/audisp/audisp-remote.conf') do
      its('disk_full_action'.to_s) {
 should cmp input('expected_disk_full_action') }
