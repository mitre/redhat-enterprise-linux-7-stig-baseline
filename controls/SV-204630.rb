...
control "SV-204630" do
  ...
  desc "Source-routed packets allow the source of the packet to suggest that 
    ...
    router, which can be used to bypass network security measures. This 
    requirement applies only to the forwarding of source-routed traffic, such 
    as when IPv6 forwarding is enabled and the system is functioning as a router."
  desc "default", "Source-routed packets allow the source of the packet to 
    ...
    traffic, such as when IPv6 forwarding is enabled and the system is functioning 
    as a router."
  desc "check", "If IPv6 is not enabled, the key will not exist, and this is 
    Not Applicable.
  ...
  desc "fix", "Set the system to the required kernel parameter, if IPv6 is enabled, 
    by adding the following line to \"/etc/sysctl.conf\" or a configuration file 
