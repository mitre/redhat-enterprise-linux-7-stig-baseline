...
control "SV-204584" do
  ...
  desc "Address space layout randomization (ASLR) makes it more difficult for 
    ...
    Additionally, ASLR also makes it more difficult for an attacker to know 
    the location of existing code in order to repurpose it using return-oriented 
    programming (ROP) techniques."
  desc "default", "Address space layout randomization (ASLR) makes it more 
    difficult for an attacker to predict the location of attack code he or 
    she has introduced into a process's address space during an attempt at 
    exploitation. Additionally, ASLR also makes it more difficult for an 
    attacker to know the location of existing code in order to repurpose it
  ...
  desc "check", "Verify the operating system implements virtual address space 
    ...
    If \"kernel.randomize_va_space\" is not configured in the /etc/sysctl.conf file 
    or in the /etc/sysctl.d/ directory, is commented out or does not have a value of 
    \"2\", this is a finding.
  ...
  desc "fix", "Configure the operating system implement virtual address space 
    ...
    \"/etc/sysctl.conf\" or a config file in the /etc/sysctl.d/ directory (or modify 
