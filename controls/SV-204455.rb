...
control "SV-204455" do
  title "The Red Hat Enterprise Linux operating system must be configured so that 
    the x86 Ctrl-Alt-Delete key sequence is disabled on the command line."
  ...
  of a mixed OS environment, this can create the risk of short-term loss of 
  ...
  desc "default", "A locally logged-on user who presses Ctrl-Alt-Delete, when 
    ...
    Ctrl-Alt-Delete sequence is reduced because the user will be prompted before 
    any action is taken."
  desc "check", "Verify the operating system is not configured to reboot the 
    system when Ctrl-Alt-Delete is pressed.

    Check that the ctrl-alt-del.target is masked and not active with the following 
    command:
