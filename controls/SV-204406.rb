...
control "SV-204406" do
  title "The Red Hat Enterprise Linux operating system must be configured so 
    that when passwords are changed or new passwords are established, 
    pwquality must be used."
  desc "Use of a complex password helps to increase the time and resources 
    required to compromise the password. Password complexity, or strength, 
    is a measure of the effectiveness of a password in resisting attempts 
    at guessing and brute-force attacks. \"pwquality\" enforces complex 
    password construction configuration and has the ability to limit 
  ...
  desc "default", "Use of a complex password helps to increase the time and 
    ...
    attempts at guessing and brute-force attacks. \"pwquality\" enforces 
    complex password construction configuration and has the ability to
    limit brute-force attacks on the system."
  desc "check", "Verify the operating system uses \"pwquality\" to enforce 
  ...
  describe pam('/etc/pam.d/system-auth') do
    its('lines') {
 should match_pam_rule("password required pam_pwquality.so retry=#{input('retry')}") }
  end
