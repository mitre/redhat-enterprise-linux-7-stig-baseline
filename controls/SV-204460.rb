...
control "SV-204460" do
  title "The Red Hat Enterprise Linux operating system must not have unnecessary 
    accounts."
  desc "Accounts providing no operational purpose provide additional opportunities 
    for system compromise. Unnecessary accounts include user accounts for 
    individuals not requiring access to the system and application accounts 
    for applications not installed on the system."
  desc "default", "Accounts providing no operational purpose provide additional 
    opportunities for system compromise. Unnecessary accounts include user 
    accounts for individuals not requiring access to the system and application
  ...
  desc "check", "Verify all accounts on the system are assigned to an active 
    ...
    accounts that do not support an authorized system function are present, 
    this is a finding."
