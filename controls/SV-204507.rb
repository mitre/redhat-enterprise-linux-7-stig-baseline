...
control "SV-204507" do
  ...
  desc "Information stored in one location is vulnerable to accidental or 
    ...
    the use of the audisp-remote dameon.  When the remote buffer is full, 
    audit logs will not be collected and sent to the central log server."
  desc "default", "Information stored in one location is vulnerable to 
    ...
    Linux is with the use of the audisp-remote dameon.  When
    the remote buffer is full, audit logs will not be collected and 
    sent to the central log server."
  desc "check", "Verify the audisp daemon is configured to take an appropriate 
  action when the internal queue is full:

    # grep \"overflow_action\" /etc/audisp/audispd.conf

    overflow_action = syslog

    If the \"overflow_action\" option is not \"syslog\", \"single\", or \"halt\", 
    ...
    to indicate what action that system takes when the internal queue is full.

    If there is no evidence the system is configured to off-load audit logs to a 
    different system or storage media or, if the configuration does not take 
