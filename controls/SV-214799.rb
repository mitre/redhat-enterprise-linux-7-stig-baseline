...
control "SV-214799" do
  title "The Red Hat Enterprise Linux operating system must be configured so 
    that the cryptographic hash of system files and commands matches vendor 
    values."
  ...
  desc "default", "Without cryptographic integrity protections, system command 
    and files can be altered by unauthorized users without detection.
    Cryptographic mechanisms used for protecting the integrity of information 
    include, for example, signed hash functions using asymmetric cryptography 
    enabling distribution of the public key to verify the hash information while
    maintaining the confidentiality of the key used to generate the hash."
  desc "check", "Verify the cryptographic hash of system files and commands match 
    the vendor values.
    ...
    If there is any output from the command for system files or binaries, this is a 
    finding."
