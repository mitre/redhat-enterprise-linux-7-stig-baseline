...
control "SV-204447" do
  title "The Red Hat Enterprise Linux operating system must prevent the 
    ...
    operating system components from a repository without verification they 
  ...
  desc "default", "Changes to any software components can have significant 
    ...
    with and that it has been provided by a trusted vendor. Self-signed 
    certificates are disallowed by this requirement. The operating system 
    should not have to verify the software again. This requirement does not 
    mandate DoD certificates for this purpose; however, the
  ...
  desc "check", "Verify the operating system prevents the installation of 
    ...
    from a repository without verification that they have been digitally 
    signed using a certificate that is recognized and approved by the 
    organization.
  ...
  desc "fix", "Configure the operating system to verify the signature of packages 
    from a repository prior to install by setting the following option in the \"/etc/yum.conf\" file:
