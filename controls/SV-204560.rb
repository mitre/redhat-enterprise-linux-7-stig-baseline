...
control "SV-204560" do
  ...
  desc "default", "Without generating audit records that are specific to the 
    ...
    filter). The system call rules are loaded into a matching engine that 
    intercepts each syscall made by all programs on the system. Therefore, it 
    is very important to use syscall rules only when absolutely necessary 
    since these affect performance. The more rules, the bigger the performance 
    hit. The performance can be helped, however, by combining
    syscalls into one rule whenever possible."
  desc "check", "Verify the operating system generates audit records upon 
    successful/unsuccessful attempts to use the \"init_module\" and 
    ...
    If both the \"b32\" and \"b64\" audit rules are not defined for the \"init_module\" 
    and \"finit_module\" syscalls, this is a finding."
  desc "fix", "Configure the operating system to generate audit records upon 
    successful/unsuccessful attempts to use the \"init_module\" and \"finit_module\" 
    syscalls. 
