...
control "SV-237635" do
  ...
  desc "Without re-authentication, users may access resources or perform tasks 
    ...
    not expire and the user will not have to re-authenticate for privileged 
  ...
  desc "default", "Without re-authentication, users may access resources or 
    ...
    expire and the user will not have to re-authenticate for privileged actions 
    until the user's session is terminated."
  desc "check", "Verify the operating system requires re-authentication when 
    using the \"sudo\" command to elevate privileges.

    $ sudo grep -i 'timestamp_timeout' /etc/sudoers /etc/sudoers.d/*
    /etc/sudoers:Defaults timestamp_timeout=0

    If results are returned from more than one file location, this is 
    a finding.
