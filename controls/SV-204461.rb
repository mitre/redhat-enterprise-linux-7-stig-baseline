...
control "SV-204461" do
  title "The Red Hat Enterprise Linux operating system must be configured so that 
    all Group Identifiers (GIDs) referenced in the /etc/passwd file are defined 
    in the /etc/group file."
  desc "If a user is assigned the GID of a group not existing on the system, and 
    a group with the GID is subsequently created, the user may have unintended 
    rights to any files associated with the group."
  desc "default", "If a user is assigned the GID of a group not existing on the 
    system, and a group with the GID is subsequently created, the user may have 
    unintended rights to any files associated with the group."
  ...
  desc "fix", "Configure the system to define all GIDs found in the \"/etc/passwd\" 
  ...
  describe "All group identifiers in /etc/passwd" do
    it "should be defined in /etc/groups" do
      expect(passwd.gids.map{ |gid| gid.to_i }).to all(be_in etc_group.gids),
        "missing gids: #{passwd.gids.select{ |gid|
 !etc_group.gids.include?(gid.to_i)}}"
    end
  end
end