...
control "SV-204587" do
  title "The Red Hat Enterprise Linux operating system must be configured so 
    that all network connections associated with SSH traffic are terminated 
    at the end of the session or after 10 minutes of inactivity, except to 
  ...
  desc "Terminating an idle SSH session within a short time period reduces the 
    ...
    at the operating system level and de-allocating networking assignments at 
  ...
  desc "default", "Terminating an idle SSH session within a short time period 
    ...
    has been left unattended. In addition, quickly terminating an idle SSH 
    ...
    operating system-level network connection. This does not mean that the 
    operating system terminates all sessions or network access; it only 
    ends the inactive session and releases the resources associated with 
    that session."
  ...
  if virtualization.system.eql?('docker') && !file('/etc/sysconfig/sshd').exist?
    ...
    describe sshd_config do
      its('ClientAliveInterval') {
 should cmp input('expected_client_alive_interval') }
      its('ClientAliveInterval') {
 should be_between(1, input('max_client_alive_interval')) }
