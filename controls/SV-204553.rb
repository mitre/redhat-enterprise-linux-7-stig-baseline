...
control "SV-204553" do
  ...
  desc "Reconstruction of harmful events or forensic analysis is not possible 
    ...
    sufficient detail to reconstruct events to determine the cause and impact 
    of compromise.
  ...
  desc "default", "Reconstruction of harmful events or forensic analysis is not 
    possible if audit records do not contain enough information.
    At a minimum, the organization must audit the full-text recording of 
    privileged mount commands. The organization must maintain audit trails 
    in sufficient detail to reconstruct events to determine the cause and impact of
    compromise. When a user logs on, the auid is set to the uid of the account 
    ...
    which equals 4294967295. The audit system interprets -1, 4294967295, 
    and \"unset\" in the same way."
  desc "check", "Verify the operating system generates audit records when 
    ...
    following series of commands to check the file system rules in \"/etc/audit/audit.rules\": 
