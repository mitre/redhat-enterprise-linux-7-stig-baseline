...
control "SV-204555" do
  ...
  desc "Reconstruction of harmful events or forensic analysis is not possible 
    ...
    postfix commands. The organization must maintain audit trails in sufficient 
  ...
  desc "default", "Reconstruction of harmful events or forensic analysis is not 
    ...
    privileged postfix commands. The organization must maintain audit trails 
    in sufficient detail to reconstruct events to determine the cause and impact 
    of compromise. When a user logs on, the auid is set to the uid of the account 
    that is being authenticated. Daemons are not user sessions and have the loginuid 
    set to -1. The auid representation is an unsigned 32-bit integer, which equals
