...
control "SV-204438" do
  title "Red Hat Enterprise Linux operating systems version 7.2 or newer with a 
    Basic Input/Output System (BIOS) must require authentication upon booting 
  ...
  desc "check", "For systems that use UEFI, this is Not Applicable.
    ...
    Check to see if an encrypted grub superusers password is set. On systems that 
    ...
    this is a finding."
  desc "fix", "Configure the system to encrypt the boot password for the grub 
    superusers account with the grub2-setpassword command, which creates/overwrites 
    the /boot/grub2/user.cfg file.

    Generate an encrypted grub2 password for the grub superusers account with the 
    following command:
