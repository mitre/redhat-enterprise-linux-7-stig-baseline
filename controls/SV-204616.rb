...
If conflicting results are returned, this is a finding.'
  ...
  if virtualization.system.eql?('docker')
    ...
    results_in_files = command('grep -r net.ipv4.conf.default.send_redirects /run/sysctl.d/* /etc/sysctl.d/* /usr/local/lib/sysctl.d/* /usr/lib/sysctl.d/* /lib/sysctl.d/* /etc/sysctl.conf 2> /dev/null').stdout.strip.split("\n")
    ...
    describe 'net.ipv4.conf.default.send_redirects' do
      it "should be set to #{send_redirects} in the configuration files" do
        conflicting_values_fail_message = "net.ipv4.conf.default.send_redirects is set to conflicting values as follows: #{unique_values}"
        incorrect_value_fail_message = "The net.ipv4.conf.default.send_redirects value is set to #{unique_values[0]} and should be set to #{send_redirects}"
        unless unique_values.empty?
          expect(unique_values.length).to cmp(1), conflicting_values_fail_message
          expect(unique_values[0]).to cmp(send_redirects), incorrect_value_fail_message
        end
      end
    end
