...
control "SV-204610" do
  ...
  desc "Enabling reverse path filtering drops packets with source addresses 
    ...
    small networks."
  desc "default", "Enabling reverse path filtering drops packets with source 
    addresses that should not have been able to be received on the interface 
    they were received on. It should not be used on systems which are routers 
  ...
  desc "check", "Verify the system uses a reverse-path filter for IPv4:
    ...
    If \"net.ipv4.conf.all.rp_filter\" is not configured in the /etc/sysctl.conf 
    file or in the /etc/sysctl.d/ directory, is commented out, or does not have 
    a value of \"1\", this is a finding.
