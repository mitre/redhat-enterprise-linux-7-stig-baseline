...
control "SV-204427" do
  ...
  desc "By limiting the number of failed logon attempts, the risk of unauthorized 
    system access via user password guessing, otherwise known as brute-forcing, 
    is reduced. Limits are imposed by locking the account."
  ...
  desc "check", "Check that the system locks an account for a minimum of 15 
    minutes after three unsuccessful logon attempts within a period of 15 
  ...
  desc "fix", "Configure the operating system to lock an account for the maximum 
    ...
    program. The \"authconfig\" program should not be used to update the configurations 
  ...
  describe.one do
    describe "pam rules for the faillock module" do
      it "should exactly match an appropriately configured ruleset in password-auth" do
        expect(pa_rules).to match_pam_rules(req).exactly, "missing required rules: #{req.select{ |rule|
 !pa_rules.include?(rule) }}"
      end
    end
    describe "pam rules for the faillock module" do
      it "should exactly match an appropriately configured ruleset in password-auth" do
        expect(pa_rules).to match_pam_rules(alt).exactly, "missing alternate rules: #{alt.select{ |rule|
 !pa_rules.include?(rule) }}"
      end
    end
  end
  describe "pam rules for the faillock module" do
    it "should have the expected settings enabled in password-auth" do
      expect(pa_rules).to match_pam_rule(deny_pattern), 
"missing: #{deny_pattern}"
      expect(pa_rules).to match_pam_rule(fail_interval_pattern), 
"missing: #{fail_interval_pattern}"
      expect(pa_rules).to match_pam_rule(unlock_time_pattern), 
"missing or misconfigured unlock_time"
    end
  end
  describe.one do
    describe "pam rules for the faillock module" do
      it "should exactly match an appropriately configured ruleset in system-auth" do
        expect(sa_rules).to match_pam_rules(req).exactly, "missing required rules: #{req.select{ |rule|
 !sa_rules.include?(rule) }}"
      end
    end
    describe "pam rules for the faillock module" do
      it "should exactly match an appropriately configured ruleset in system-auth" do
        expect(sa_rules).to match_pam_rules(alt).exactly, "missing alternate rules: #{alt.select{ |rule|
 !sa_rules.include?(rule) }}"
      end
    end
  end
  describe "pam rules for the faillock module" do
    it "should have the expected settings enabled in system-auth" do
      expect(sa_rules).to match_pam_rule(deny_pattern), 
"missing: #{deny_pattern}"
      expect(sa_rules).to match_pam_rule(fail_interval_pattern), 
"missing: #{fail_interval_pattern}"
      expect(sa_rules).to match_pam_rule(unlock_time_pattern), 
"missing or misconfigured unlock_time"
    end
  end
