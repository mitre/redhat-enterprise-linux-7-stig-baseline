...
Note: Per requirement RHEL-07-010199, RHEL 7 must be configured to not overwrite custom authentication configuration settings while using the authconfig utility, otherwise manual changes to the listed files will be overwritten whenever the authconfig utility is used.'
  ...
  # deny_pattern = faillock_rule_pattern + " deny=#{input('unsuccessful_attempts')}" #3
  # fail_interval_pattern = faillock_rule_pattern + " fail_interval=#{input('fail_interval')}" #900
  # unlock_time_pattern = faillock_rule_pattern + " unlock_time=(0|never|#{input('lockout_time')})" #604800
  ...
  describe file('/etc/pam.d/password-auth') do
    its('content') { should match /auth        required      pam_faillock.so preauth deny=3 even_deny_root fail_interval=900 unlock_time=0/ }
    its('content') { should match /auth        required      pam_faillock.so authfail deny=3 even_deny_root fail_interval=900 unlock_time=0/ }
    its('content') { should match /account     required      pam_faillock.so/ }
    it { should_not match /#.*pam_faillock.so/ }
  end
