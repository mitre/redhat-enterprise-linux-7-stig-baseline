...
control "SV-204596" do
  ...
  desc "If a public host key file is modified by an unauthorized user, the 
    SSH service may be compromised."
  desc "default", "If a public host key file is modified by an unauthorized 
    user, the SSH service may be compromised."
  desc "check", "Verify the SSH public host key files have mode \"0644\" or 
    less permissive.
    ...
    if !pub_files.nil? and !pub_files.empty?
      pub_files.each do |pubfile|
        describe file(pubfile) do
          it {
 should_not be_more_permissive_than(input('public_host_key_file_mode')) }
        end
      end
    else
