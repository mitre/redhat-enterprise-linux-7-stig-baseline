...
control "SV-250313" do
  title "The Red Hat Enterprise Linux operating system must not allow privileged 
    accounts to utilize SSH."
  ...
  desc "check", "Note: Per OPORD 16-0080, the preferred endpoint security tool 
    ...
    If the \"ssh_sysadm_login\" boolean is not \"off\" and is not documented 
    with the ISSO as an operational requirement, this is a finding."
  desc "fix", "Configure the operating system to prevent privileged accounts 
    from utilizing SSH.
    Use the following command to set the \"ssh_sysadm_login\" boolean to \"off\":

    $ sudo setsebool -P ssh_sysadm_login off

    Note: SELinux confined users mapped to sysadm_u are not allowed to login 
    to the system over SSH, by default. If this is a required function, it can 
    be configured by setting the ssh_sysadm_login SELinux boolean to \"on\" with 
