...
control "SV-204619" do
  ...
  desc "default", "If unrestricted mail relaying is permitted, unauthorized 
    senders could use this host as a mail relay for the purpose of sending 
    spam or other unauthorized activity."
  ...
  if package('postfix').installed?
    ...
    if (defined? parse_config_file('/etc/postfix/main.cf', 
options).params['smtpd_client_restrictions'])then
      pf_config = parse_config_file('/etc/postfix/main.cf', 
options).params['smtpd_client_restrictions'].split(',')
    end
    describe "Postfix config setting smptd_client_restrictions" do
      it "should be set to 'permit_mynetworks', 'reject', or both" do
        expect(pf_config).to all satisfy { |x|
 ['permit_mynetworks', 'reject'].include?(x) }
      end
    end
  else
