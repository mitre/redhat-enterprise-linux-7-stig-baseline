...
control "SV-204489" do
  ...
  desc "Cron logging can be used to trace the successful or unsuccessful 
    execution of cron jobs. It can also be used to spot intrusions into 
    the use of the cron facility by unauthorized and malicious users."
  desc "default", "Cron logging can be used to trace the successful or 
    unsuccessful execution of cron jobs. It can also be used to spot 
    intrusions into the use of the cron facility by unauthorized and 
    malicious users."
  desc "check", "Verify that \"rsyslog\" is configured to log cron events.

    Check the configuration of \"/etc/rsyslog.conf\" or \"/etc/rsyslog.d/*.conf\" 
    files for the cron facility with the following command:
    ...
    If the command does not return a response, check for cron logging all facilities 
    by inspecting the \"/etc/rsyslog.conf\" or \"/etc/rsyslog.d/*.conf\" files.
  ...
  desc "fix", "Configure \"rsyslog\" to log all cron messages by adding or 
    updating the following line to \"/etc/rsyslog.conf\" or a configuration 
    file in the /etc/rsyslog.d/ directory:
  ...
  describe.one do
    ...
    describe "All facilities" do
      it "should be configured for logging in the logging utility config files" do
        expect(facilities_log).to match(%r{^.+:\*\.\*\s+/var/log/messages}), 
"cron not found in #{log_pkg_paths}"
      end
    end
  end
end