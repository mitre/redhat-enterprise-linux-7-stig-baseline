...
control "SV-204422" do
  title "The Red Hat Enterprise Linux operating system must be configured so that 
    passwords are prohibited from reuse for a minimum of five generations."
  desc "Password complexity, or strength, is a measure of the effectiveness of a 
    ...
    information system or application allows the user to consecutively reuse 
    their password when that password has exceeded its defined lifetime, the 
  end result is a password that is not changed per policy requirements."
  desc "default", "Password complexity, or strength, is a measure of the 
    effectiveness of a password in resisting attempts at guessing and 
    brute-force attacks. If the information system or application allows 
    the user to consecutively reuse their password when that password has 
    exceeded its defined lifetime, the end result is a password that is not
  ...
  desc "check", "Verify the operating system prohibits password reuse for a 
    ...
    Check for the value of the \"remember\" argument in \"/etc/pam.d/system-auth\" 
    and \"/etc/pam.d/password-auth\" with the following command:

    # grep -i remember /etc/pam.d/system-auth /etc/pam.d/password-auth

    password    requisite     pam_pwhistory.so use_authtok remember=5 retry=3

    If the line containing the \"pam_pwhistory.so\" line does not have the \"remember\" 
  ...
  desc "fix", "Configure the operating system to prohibit password reuse for a 
    ...
    Add the following line in \"/etc/pam.d/system-auth\" and \"/etc/pam.d/password-auth\" 
  ...
  describe.one do
    describe pam('/etc/pam.d/system-auth') do
      its('lines') {
 should match_pam_rule("password (required|requisite|sufficient) pam_(unix|pwhistory).so remember=#{reuse_generations}") }
    end
