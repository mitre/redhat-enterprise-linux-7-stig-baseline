...
control "SV-204516" do
  title "The Red Hat Enterprise Linux operating system must audit all executions 
    of privileged functions."
  desc "Misuse of privileged functions, either intentionally or unintentionally 
    by authorized users, or by unauthorized external entities that have 
  ...
  desc "default", "Misuse of privileged functions, either intentionally or 
    ...
    that have compromised information system accounts, is a serious and ongoing 
    concern and can have significant adverse impacts on organizations. Auditing 
    the use of privileged functions is one way to detect such misuse and identify 
  ...
  desc "check", "Verify the operating system audits the execution of privileged 
    ...
    If both the \"b32\" and \"b64\" audit rules for \"SUID\" files are not defined, 
    this is a finding.

    If both the \"b32\" and \"b64\" audit rules for \"SGID\" files are not defined, 
    this is a finding."
    ...
    describe "Syscall" do
      audit_syscalls.each do |audit_syscall|
        it "#{audit_syscall} is audited properly" do
          ...
          expect(audit_rule.fields.flatten).to include('uid!=euid','gid!=egid', 
'euid=0', 'egid=0')
