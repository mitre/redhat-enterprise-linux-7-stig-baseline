...
control "SV-204498" do
  title "The Red Hat Enterprise Linux operating system must be configured so 
    that the file integrity tool is configured to verify Access Control Lists (ACLs)."
  ...
  desc "default", "ACLs can provide permissions beyond those permitted through 
    the file mode and must be verified by file integrity tools."
  desc "check", "Verify the file integrity tool is configured to verify ACLs.
    ...
    If there is no application installed to perform file integrity checks, 
    this is a finding.
